...
@elseif(isset($type) && $type == 'text_area')
    <div class="col-sm-12">
        <div class="input-material-group mb-3">
            <textarea style="padding: 5px 10px" class="input-material{{ $errors->has($field)?' validation_error':'' }}" placeholder="{{ $fieldTitle }}" id="{{ $field }}" row="1"
...
@elseif(isset($type) && $type == 'yes_no')
    <div class="col-sm-12">
        <div class="input-material-group mb-3">
            <select required name="{{ $field }}" class="input-material{{ $errors->has($field)?' validation_error':'' }}"
                    id="{{ $field }}">
                <option value="" disabled selected>{{ $fieldTitle }}</option>
                <option {{ (old($field) == 'yes') ? 'selected' : '' }} value="yes">Yes</option>
                <option {{ (old($field) == 'no') ? 'selected' : '' }} value="no">No</option>
            </select>
            @if($errors->has($field))
                <div class="js-validate-error-label" style="color: #B81111">{{ $errors->first($field) }}</div>
            @else
    ...
    <div class="col-sm-12">
        <div class="input-material-group mb-3">
            <select required name="{{ $field }}" class="input-material{{ $errors->has($field)?' validation_error':'' }}"
                    id="{{ $field }}">
                <option value="" disabled selected>{{ $fieldTitle }}</option>
                <option {{ (old($field) == 'active') ? "selected" : "" }} value="active">Active</option>
                <option {{ (old($field) == 'de_active') ? "selected" : "" }} value="de_active">De-active</option>
            </select>
            @if($errors->has($field))
                <div class="js-validate-error-label" style="color: #B81111">{{ $errors->first($field) }}</div>
            @else
