                ...
                <div class="col-lg-12">
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home" aria-selected="true">Basic detail</button>
                            <button class="nav-link" id="nav-home-tab-two" data-bs-toggle="tab" data-bs-target="#nav-home-two" type="button" role="tab" aria-controls="nav-home" aria-selected="true">Locations</button>
                            <button class="nav-link" id="nav-bid-tab-two" data-bs-toggle="tab" data-bs-target="#nav-bid-two" type="button" role="tab" aria-controls="nav-bid" aria-selected="true">Bids</button>
                            @if($row->status == 'delivery' || $row->status == 'picked_up' || ($row->status == 'on_site' && $row->admin_status_approved == 0))
                            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button" role="tab" aria-controls="nav-profile" aria-selected="false">Picked up detail</button>
                            @endif
                            @if($row->status == 'delivery' || ($row->status == 'picked_up' && $row->admin_status_approved == 0))
                            <button class="nav-link" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-contact" type="button" role="tab" aria-controls="nav-contact" aria-selected="false">Delivery detail</button>
                                @endif
                        </div>
                    </nav>
                    <div class="tab-content" id="nav-tabContent">
                        ...
                        <div class="tab-pane fade" id="nav-bid-two" role="tabpanel" aria-labelledby="nav-bid-tab-two">
                            <div class="card mb-0">
                                <div class="card-body d-flex flex-column">
                                    <h3>{{ $row->first_name.' '.$row->last_name }}</h3>
                                    <table class="table">
                                        ...
                                        <tbody>
                                        @foreach($row->bidingUsers as $bidUser)
                                            <tr>
                                                <td>{{ $bidUser->driver->full_name }}</td>
                                                <td>{{ $bidUser->amount }}</td>
                                                <td>
                                                    <form action="{{ route('super_admin.bid.approved',$bidUser->id) }}" method="post">
                                                        @csrf
                                                        <button class="btn btn-success btn-xs" type="submit"><i class="fa fa-check"></i> Approve</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
...
{{--                                                            <a href="{{ $image->image("upload") }}" target="_blank"><img src="{{ $image->image("300_300") }}" style="height: 100px" alt=""></a>--}}
{{--                                                        @endforeach--}}
{{--                                                        @endif--}}
                                                </td>
                                            <tr>
