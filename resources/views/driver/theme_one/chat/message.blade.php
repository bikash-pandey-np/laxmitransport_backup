...
@section('breadcrumb')
    <nav aria-label="breadcrumb">
        <div style="padding: 10px;">
            <a href="{{ route('driver.'.request('role').'.show',$user->id) }}" style="color: #2f333e">
                {{ $user->full_name }}
            </a>

            <a href="{{ route('driver.index') }}" style="float: right;color: #2f333e">
        ...
        <div class="container-fluid">
            <div class="row gy-2">
                    <div style="display: flex;">
                        <input class="chat_send_message" style="color: black" />
...
@push('js')
                ...
                function messageSend(){
                    ...
                    <div class="receiver">
                        ...
                        <img class="avatar shadow-0 img-fluid rounded-circle chat-profile" src="{{ auth()->user()->image('50_50') }}"/>
                            ...
                            result.sender_name = "{{ auth('driver')->user()->full_name ?? "" }}";
                            $("#message_list").animate({scrollTop: 20000000}, "slow");
                            socket.emit('message', {
    ...
    <script>
        ...
        socket.on("{{ auth('driver')->id() }}"+'drivermessage_listen',function (data) {
            if (data.user_type == "driver" && data.id == "{{ request('id') }}"){
                $("#message_list").append(`<div class="col-lg-12">
                    <div class="sender">
                        <img class="avatar shadow-0 img-fluid rounded-circle chat-profile"
    ...
    <script>
            $.get('{{ url('driver/chat/user/list') }}',function (response) {
                var html = "";
                $.each(response,function(index,value){

                    var route = "{{ url('driver/chat') }}/"+value.type+"/"+value.id;

                    html = html + `<li class="sidebar-item{{ Request::is('driver/chat/'.'2')?' active':'' }}">
