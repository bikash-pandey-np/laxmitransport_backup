...
class WorkController extends DriverBaseController
{
    ...
    public function status($status)
    {
        ...
        if (request('status') == 'delivery') {
            $works = Work::with("workLocations")->whereIn('vehicle_id', $vehicles)->whereIn('status', ['Unloaded','Cancel'])->orderBy('id', 'desc')->get();
        } else {
            $works = Work::with("workLocations")->whereIn('vehicle_id', $vehicles)->whereNotIn('status', [ 'Unloaded','Cancel'])->orderBy('id', 'desc')->get();
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('driver.work.index', 'on_site')->with('success', 'Update Successful');
    }
}
