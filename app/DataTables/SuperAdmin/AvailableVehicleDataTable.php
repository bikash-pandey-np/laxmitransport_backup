...
class AvailableVehicleDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('date',function($query){
                ...
                return "";
                $pickup = (isset($query->latestAllWork->workLocation->pickup_date_time)) ? "<p class='btn btn-primary'>".format_server_to_local($query->latestAllWork->workLocation->pickup_date_time)."</p>" : "";
                $drop = (isset($query->latestAllWork->workLocation->drop_date_time)) ? "<p class='btn btn-secondary'>".format_server_to_local($query->latestAllWork->workLocation->drop_date_time)."</p>" : "";
            ...
            ->addColumn('driver_status', function ($query) {
                if (($query->driver->admin_approve ?? 0) == 0) {
                    return '<a href="' . route('driver.active.by.admin', $query->driver->id??1) . '">Approve this user</a>';
                }
    ...
    {
        $notAvailableIds = $model->orderBy('vehicles.created_at','desc')
            ->select([
                'vehicles.*'
            ])
            ...
            ->pluck('id')->toArray();
        return $model->whereNotIn('vehicles.id',$notAvailableIds)
            ->select('vehicles.*')
